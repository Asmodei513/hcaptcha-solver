...
Инструмент для автоматического решения hCaptcha на странице входа Steam. Поддерживает несколько стратегий решения с автоматическим переключением при неудаче.

### Возможности

🎤 **Аудио-челленджи** — распознавание речи через Google Speech API, Whisper (локально) или Vosk (оффлайн)

🖼️ **Картинки** — классификация изображений через HuggingFace API, Imagga или локальную ResNet50

🌐 **Сторонние API** — 2Captcha и AntiCaptcha как надёжный fallback (человек решает за вас)

🔐 **Steam** — полный flow логина с поддержкой Steam Guard 2FA, сохранением сессий и куков

🛡️ **Антидетект** — реалистичный fingerprint браузера, скрытие webdriver, имитация поведения человека

🌐 **Прокси** — поддержка HTTP/SOCKS прокси с авторизацией

### Установка

```bash
git clone https://github.com/Asmodei513/hcaptcha-solver.git
cd hcaptcha-solver
...
```
Аудио-челлендж → Картинки → 2Captcha API → AntiCaptcha API
```

Каждый метод пробуется по порядку. Если один не сработал — следующий.
...
Made with ❤️ by [Asmodei513](https://github.com/Asmodei513)
